...
Brain::Brain() : rclcpp::Node("brain_node")
{
    ...
    declare_parameter<bool>("game.half_court_mode", false);
...
void Brain::loadConfig()
{
    ...
    get_parameter("game.half_court_mode", config->halfCourtMode);
...
void Brain::handleCooperation() {
    ...
    // For single robot mode (no communication), automatically set as lead
    if (!config->enableCom) {
        data->tmImLead = true;
        log_("Single robot mode: automatically set as lead");
        tree->setEntry<bool>("is_lead", true);
        return; // Skip rest of cooperation logic
    }
